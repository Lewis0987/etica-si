...
################ELK(請輸入)###########################
url = "https://in-game-elk.xu-jie-tech.com/login"
################ELK(請輸入)###########################
# 開啟網頁，並最大化視窗
start = time.time()  # ✅ 開始計時（打開網頁前）
driver.get(url)      # 操作：打開網頁
end = time.time()    # ✅ 結束計時（打開完成後）
driver.maximize_window()

# 等待某些元素出現（示範用法）
WebDriverWait(driver, 10)  # 這行通常要接 `.until(...)` 來等一個條件
...
password_field.send_keys("q7s2s39im2SYAd59p42kv3ay") #舊njhq9RPmzI4KlFpW41jld1pAi0Lltr1J
print("2.密碼  \033[32mOK\033[0m")
#登入
login_button = WebDriverWait(driver, 10).until(
    EC.element_to_be_clickable((By.CSS_SELECTOR, "[data-test-subj='loginSubmit']"))
)
login_button.click()
...
sidemeun_Discover = WebDriverWait(driver, 10).until(
    EC.element_to_be_clickable((By.XPATH, "//*[contains(@href, 'https://in-game-elk.xu-jie-tech.com/app/discover#/')]"))
)
